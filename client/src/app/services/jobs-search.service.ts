...
export class JobsSearchService {
  ...
  constructor(private http: HttpClient) { }
   ...
   getJobs(str: string):any {
    // const url = 'https://jsearch.p.rapidapi.com/search?query='+encodeURIComponent(str)+'&page=1&num_pages=1';
    // let result;
    // const options = {
    //   method: 'GET',
    //   headers: {
    //     'X-RapidAPI-Key': environment.keyJob,
    ...
    //    result = await (response.text() as object);
    //   console.log(result);
    // } catch (error) {
    //   console.error(error);
